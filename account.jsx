...
function AccountPage({ go, route, store }) {
  ...
  const tabs = [["trips", t("acc_trips"), "compass", store.bookings.length], ["saved", t("acc_saved"), "heart", store.saved.length], ["profile", t("acc_profile"), "user"]];
      ...
      <div style={{ background: "linear-gradient(160deg, var(--ocean-deep), var(--ocean))", color: "white", position: "relative", overflow: "hidden" }}>
        ...
        <div className="wrap" style={{ position: "relative", zIndex: 2, padding: "40px 28px 30px" }}>
          <div className="row gap-4" style={{ flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1 }}>
              <h1 className="display" style={{ fontSize: "2.4rem" }}>{t("acc_welcome")} {user.name.split(" ")[0]}</h1>
              <p style={{ color: "oklch(1 0 0 / 0.82)" }}>{user.email}</p>
            </div>
            <button className="btn" onClick={() => { Store.signOut(); go({ view: "home" }); }} style={{ background: "oklch(1 0 0 / 0.16)", color: "white", backdropFilter: "blur(8px)" }}>{t("acc_signout")}</button>
          </div>
        </div>
      </div>
...
function TripsTab({ store, go }) {
  const { t } = useI18n();
  if (!store.bookings.length) return <EmptyState icon="compass" title={t("acc_no_trips_h")} text={t("acc_no_trips_p")} cta={t("acc_explore")} onCta={() => go({ view: "listing" })} />;
  return (
          ...
          <div key={b.id} className="card" style={{ display: "flex", boxShadow: "var(--sh-sm)", opacity: cancelled ? 0.6 : 1, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, padding: 24, minWidth: 260 }}>
              ...
              <div className="row gap-4" style={{ color: "var(--ink-2)", fontSize: "0.88rem", flexWrap: "wrap", marginBottom: 16 }}>
                ...
                <span className="row gap-2"><Icon name="users" size={15} /> {b.travellers} {t("acc_journeys")}</span>
                <span className="row gap-2"><Icon name="tag" size={15} /> {fmtPrice(b.total)}</span>
              </div>
              <div className="row gap-3" style={{ flexWrap: "wrap" }}>
                <button className="btn btn-soft btn-sm" onClick={() => go({ view: "tour", id: b.tourId })}>{t("acc_view_itin")}</button>
                {!cancelled && <button className="btn btn-ghost btn-sm" onClick={() => Store.cancelBooking(b.id)}><Icon name="x" size={15} /> {t("acc_cancel")}</button>}
                {!cancelled && <span className="row gap-2" style={{ marginLeft: "auto", fontSize: "0.82rem", color: "var(--ink-3)" }}><Icon name="shield" size={14} /> {fmtPrice(b.total - b.deposit)} {t("acc_balance")}</span>}
              </div>
            </div>
          </div>
        );
...
function SavedTab({ store, go }) {
  ...
  if (!saved.length) return <EmptyState icon="heart" title={t("acc_no_saved_h")} text={t("acc_no_saved_p")} cta={t("acc_find")} onCta={() => go({ view: "listing" })} />;
  return (
    <div>
      <h2 className="display" style={{ fontSize: "1.8rem", marginBottom: 20 }}>{saved.length} {t("acc_saved_label")} {saved.length === 1 ? t("acc_journey") : t("acc_journeys")}</h2>
      <div className="grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
        {saved.map((tr) => <TourCard key={tr.id} tour={tr} onOpen={(id) => go({ view: "tour", id })} />)}
      </div>
    </div>
  );
}
...
function ProfileTab({ user }) {
  ...
  const prefs = [
    { icon: "diamond", l: t("cat_luxury") }, { icon: "mountain", l: t("cat_adventure") }, { icon: "leaf", l: "Sustainable travel" },
  ];
  return (
    <div className="profile-grid" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 28, alignItems: "start" }}>
      <div className="card" style={{ padding: 30, boxShadow: "var(--sh-sm)" }}>
        <h2 style={{ fontSize: "1.3rem", marginBottom: 20 }}>{t("acc_personal")}</h2>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div className="field" style={{ gridColumn: "span 2" }}><label>{t("acc_name")}</label><input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
          <div className="field" style={{ gridColumn: "span 2" }}><label>{t("acc_email")}</label><input className="input" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div>
          <div className="field"><label>{t("acc_phone")}</label><input className="input" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+1 555 000 0000" /></div>
          <div className="field"><label>{t("acc_country")}</label><select className="select" value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })}>{["United States","United Kingdom","Canada","Australia","Other"].map((c) => <option key={c}>{c}</option>)}</select></div>
        </div>
        <button className="btn btn-ocean" style={{ marginTop: 22 }} onClick={() => { Store.signIn({ ...user, name: form.name, email: form.email }); toast(t("acc_save"), "checkC"); }}>{t("acc_save")}</button>
      </div>
      <div className="col gap-4">
        <div className="card" style={{ padding: 26, boxShadow: "var(--sh-sm)" }}>
          <h3 style={{ fontSize: "1.1rem", marginBottom: 14 }}>{t("acc_prefs")}</h3>
        ...
        <div className="card" style={{ padding: 26, boxShadow: "none", background: "linear-gradient(160deg, var(--coral-soft), var(--sand))" }}>
          ...
          <h3 style={{ fontSize: "1.1rem", margin: "8px 0 4px" }}>{t("acc_circle")}</h3>
          <p style={{ fontSize: "0.88rem", color: "var(--ink-2)", lineHeight: 1.5 }}>{t("acc_circle_text")}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr" }} className="auth-wrap">
      ...
      <div style={{ display: "grid", placeItems: "center", padding: 40 }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          ...
          <h1 className="display" style={{ fontSize: "2.4rem", marginBottom: 6 }}>{mode === "signin" ? t("auth_welcome") : t("auth_create")}</h1>
          <p style={{ color: "var(--ink-2)", marginBottom: 26 }}>{mode === "signin" ? t("auth_signin_sub") : t("auth_signup_sub")}</p>
          <div className="col gap-4">
            {mode === "signup" && <div className="field"><label>{t("auth_name")}</label><input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Jordan Rivera" /></div>}
            <div className="field"><label>{t("auth_email")}</label><input className="input" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@email.com" /></div>
            <div className="field"><label>{t("auth_pass")}</label><input className="input" type="password" value={form.pass} onChange={(e) => setForm({ ...form, pass: e.target.value })} placeholder="••••••••" /></div>
            <button className="btn btn-ocean btn-lg btn-block" onClick={submit}>{mode === "signin" ? t("auth_signin_btn") : t("auth_signup_btn")}</button>
          </div>
          <div className="row gap-3" style={{ margin: "20px 0" }}><span className="hr" /><span style={{ color: "var(--ink-3)", fontSize: "0.84rem", whiteSpace: "nowrap" }}>{t("auth_or")}</span><span className="hr" /></div>
          <button className="btn btn-ghost btn-block" onClick={() => { Store.signIn({ name: "Jordan Rivera", email: "jordan@email.com" }); toast("Signed in as guest", "check"); }}><Icon name="globe" size={18} /> {t("auth_guest")}</button>
          <p style={{ textAlign: "center", marginTop: 22, color: "var(--ink-2)", fontSize: "0.9rem" }}>
            {mode === "signin" ? t("auth_new") + " " : t("auth_have") + " "}
            <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")} style={{ color: "var(--ocean)", fontWeight: 700 }}>{mode === "signin" ? t("auth_create_link") : t("auth_signin_link")}</button>
          </p>
        </div>
      </div>
    </div>
